...
func (j *Json) Merge(s *Json) {
	...
	j.data = mergemap.Merge(self, src)
}
...
// Del modifies `Json` map by deleting `branch` if it is present.
func (j *Json) DelPath(branch []string) *Json {
	...
	pb := branch[0 : len(branch)-1]
	k := branch[len(branch)-1]
	pj := j.GetPath(pb...)
	v := pj.Get(k)
	pj.Del(k)
...
func (j *Json) Int32Array() ([]int32, error) {
	arr, err := j.Array()
	if err != nil {
		return nil, err
	}
